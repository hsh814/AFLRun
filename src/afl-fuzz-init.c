...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  fn = alloc_printf("%s/memory/neg", afl->out_dir);
  if (delete_files(fn, NULL)) { goto dir_cleanup_failed; }
  ck_free(fn);

  fn = alloc_printf("%s/memory/pos", afl->out_dir);
  if (delete_files(fn, NULL)) { goto dir_cleanup_failed; }
  ck_free(fn);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  /* Valuations */
  tmp = alloc_printf("%s/memory", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  tmp = alloc_printf("%s/memory/neg", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
